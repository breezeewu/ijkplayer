...
#define JNI_CLASS_RECORD_API "tv/danmaku/ijk/media/recorder/VavaRecorder"

#define JAVA_FUNC_NAME "nativeEventNotify"
#define JAVA_FUNC_PARAM "(III)V"
jmethodID		g_NativeEventNotify = 0;
jclass			g_VavaRecord_class_id = 0;
jclass			g_VavaRecord_global_class_id = 0;
extern JavaVM* g_jvm;
...
jint MediaRecord_Set_ID(JNIEnv *env, jobject thiz, struct avrecord_context* prc)
{
	...
	lbtrace("ret:%d = lbjni_set_long_field_id_from_java(env:%p, jobject thiz:%p, field_record_id:%d, (jlong)prc:%x)\n", ret, env, thiz, field_record_id, prc);
	return ret;
}
...
static jint MediaRecord_Open(JNIEnv *env, jobject thiz, /* jobject weakthiz,*/ jstring srcurl, jstring sinkurl, jstring tmpurl)
{
	...
	char* ptmpurl = NULL;
	struct avrecord_context* prc = NULL;
	struct avrecord_context* prcex = NULL;
	lbtrace("%s(srcurl:%p, sinkurl:%p, tmpurl:%p) begin, prc:%p, sizeof(struct avrecord_context*):%d\n", __func__, srcurl, sinkurl, tmpurl, prc, sizeof(struct avrecord_context*));
	if(srcurl)
	{
	...
	prcex = avrecord_open_contextex(&prc, psrcurl, psinkurl, "mov", ptmpurl);
	//prc = (struct avrecord_context*)id;
	lbtrace("prcex:%p = lavrecord_open_contextex(prc:%p, psrcurl:%s, psinkurl:%s, ptmpurl:%s)\n", prcex, prc, psrcurl, psinkurl, ptmpurl);
	//avrecord_close_contextp(&prc);
    //JNI_CHECK_GOTO(psinkurl, env, "java/lang/OutOfMemoryError", "mpjni: monstartup: ptmpurl.string oom", LABEL_RETURN);
	//jint rid = (jint)avrecord_open_context(psrcurl, psinkurl, "mov", ptmpurl);
    //prc = (struct avrecord_context*)rid;
	//lbtrace("prc:%p = avrecord_open_context(psrcurl:%s, psinkurl:%s, mov, ptmpurl:%s), rid:%x\n", prc, psrcurl, psinkurl, ptmpurl, rid);
    if(NULL == prc)
    {
...
static void MediaRecord_Stop(JNIEnv *env, jobject thiz, jint bcancel)
{
	...
	jint org_this = avrecord_set_callback(prc, post_event, NULL);
	jobject weak_thiz = (jobject)org_this;
	lbdebug("weak_thiz:%p = (jobject)avrecord_set_callback(prc, post_event, NULL), org_this:%x\n", weak_thiz, org_this);
...
int Record_Api_global_init(JNIEnv *env)
{
    int ret = 0;
	...
	lbtrace("%s(env:%p) end\n", __func__, env);
	//InitObject(env, JNI_CLASS_RECORD_API);
	LoadJavaEventNotify(g_jvm);
    return ret;
}

void Record_API_global_deinit(JNIEnv *env)
{

...
static void
IjkMediaPlayer_DeliverPacket(JNIEnv *env, jobject thiz, jint mt, jbyteArray jdatabuf, jlong pts_in_ms)
{
    //__android_log_print(ANDROID_LOG_INFO, "IjkMediaPlayer", "%s， pts：%ld\n", __func__, pts);
    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
    //__android_log_print(ANDROID_LOG_INFO, "IjkMediaPlayer", "%s， before GetByteArrayElements\n", __func__);
    jboolean isCopy;
    jbyte* pdata = NULL;
    jsize buflen = 0;
    if(jdatabuf)
    ...
    //__android_log_print(ANDROID_LOG_INFO, "IjkMediaPlayer", "%s， before ijkmp_deliver_packet\n", __func__);
    jint ret = ijkmp_deliver_packet(mp, mt, pdata, buflen, pts_in_ms*1000);
    //__android_log_print(ANDROID_LOG_INFO, "IjkMediaPlayer", "ret:%d = ijkmp_deliver_packet(mp, mt, pdata, buflen, pts)\n", ret);
    //__android_log_print(ANDROID_LOG_INFO, "IjkMediaPlayer", "%s， before ReleaseByteArrayElements\n", __func__);
    (*env)->ReleaseByteArrayElements(env, jdatabuf, pdata, 0);
    //__android_log_print(ANDROID_LOG_INFO, "IjkMediaPlayer", "%s， after ReleaseByteArrayElements\n", __func__);
    //JNI_CHECK_GOTO(mp, env, NULL, "mpjni: IjkMediaPlayer_deliver_packet buflen:%d", buflen);
...
// add by dawson
static jint IjkMediaPlayer_Start_Echo_Cancel(JNIEnv *env, jobject thiz, jint channel, jint samplerate, jint sampleformat, jint sbsamples, jint usdelay)
{
		MPTRACE(ANDROID_LOG_INFO, "IjkMediaPlayer", "%s(channel:%d, samplerate:%d, sampleformat:%d, sbsamples:%d, usdelay:%d)\n", __func__, channel, samplerate, sampleformat, sbsamples, usdelay);
		IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
		ALOGD("before ijkmp_start_echo_cancel\n");
    jint ret = ijkmp_start_echo_cancel(mp, channel, samplerate, sampleformat, sbsamples, usdelay);
    ALOGD("ret:%d = ijkmp_start_echo_cancel\n", ret);
    ijkmp_dec_ref_p(&mp);
    return ret;
}
...
{
	MPTRACE(ANDROID_LOG_INFO, "IjkMediaPlayer", "%s()\n", __func__);
	IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
	jint isCopy = 0;
  ...
  (*env)->ReleaseByteArrayElements(env, pcmdata, ppcmdata, 0);
  (*env)->ReleaseByteArrayElements(env, outdata, poutdata, 0);
  ijkmp_dec_ref_p(&mp);
  return outdata;
}
...
{
    //MPTRACE("%s(encoderid:%0x, pts:%ld) begin\n", __func__, encoderid, pts);
	int isCopy = 0;
...
static jbyteArray IjkMediaEncoder_H26x_KeyFrame_To_JPG(JNIEnv *env, jobject thiz, int codec_id, jbyteArray h264data)
{
    int isCopy = 0;
    jbyte* ph264data = (*env)->GetByteArrayElements(env, h264data, &isCopy);
    int h264datalen = (*env)->GetArrayLength(env, h264data);
    char* pjpg = (char*)malloc(1024*1024);
    int jpglen = convert_h26x_to_jpg(codec_id, ph264data, h264datalen, pjpg, 1024*1024);
    if(jpglen > 0)
    {
        jbyteArray encdata = (*env)->NewByteArray(env, jpglen);
        jbyte* pencdata = (*env)->GetByteArrayElements(env, encdata, &isCopy);
...
/*static jlong
IjkMediaPlayer_open_record(JNIEnv *env, jobject thiz, jstring recordurl)
{
    MPTRACE("%s\n", __func__);
    ...
    jlong recordid = (jlong)record_open(mp, precordurl);
    ALOGD("ijkmp_save_image_jpg: pjpgpath:%s, recordid:%ld\n", precordurl, recordid);
...
static void
IjkMediaPlayer_read_frame(JNIEnv *env, jobject thiz, jlong recordid, jbyteArray jdatabuf)
{
    MPTRACE("%s\n", __func__);
    jint mt = -1;
    jint size = -1;
    jlong pts = 0;
    read_frame((void*)recordid, &mt, NULL, &size, &pts);
    (*env)->NewByteArray(env, size);
    env->NewByteArray()
    jint mt = -1;
   jdatabuf
    read_frame((void*)recordid, );
    ALOGD("ijkmp_stop_record\n");
}


static void
IjkMediaPlayer_close_record(JNIEnv *env, jobject thiz, jlong recordid)
{
    MPTRACE("%s\n", __func__);
    
    record_close((void*)recordid);
    ALOGD("ijkmp_stop_record\n");
}*/
// ----------------------------------------------------------------------------
...
static JNINativeMethod g_methods[] = {
    ...
    { "nativeDeliverPacket",     	"(I[BJ)V",               	(void *) IjkMediaPlayer_DeliverPacket },
...
//static void IjkMediaPlayer_Stop_Echo_Cancel(JNIEnv *env, jobject thiz)
    ...
    { "nativeEchoCancel",     		"([B)[B",  								(void *) IjkMediaPlayer_Echo_Cancel },
    ...
    { "nativeConvertH26xToJPG",     "(I[B)[B",                                     (void *) IjkMediaEncoder_H26x_KeyFrame_To_JPG },

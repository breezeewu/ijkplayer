...
#define         IJK_PLAYER_VERSION_MAYJOR       0
#define         IJK_PLAYER_VERSION_MINOR        5
#define         IJK_PLAYER_VERSION_MICRO        2
#define         IJK_PLAYER_VERSION_TINY         6
...
#define sv_deinit_log()     if(g_plogctx) close_log_contex(&g_plogctx);
...
#define sv_memory(level, pmen, size, format)    log_memory(g_plogctx, level, NULL, __FILE__, __LINE__, __FUNCTION__, pmen, size, format)

#define lbdebug(...) //sv_trace
 ...
 * @return 日志上下文结构体
 **********************************************************************************************************/
// 返回表示自1970年1月1日00:00:00（国际标准时间）以来到时间记录所要表达的时间所经过的秒数
static int get_file_modify_time(const char* pfileurl)
{
    FILE* pfile = fopen(pfileurl, "r");
    if(pfile)
    {
        struct stat buf;
        int fd = fileno(pfile);
...
static void clean_overdur_log(log_ctx* plogctx, int hours)
{
    if(NULL == plogctx || NULL == plogctx->plog_dir)
    {
        return ;
    }
    int cur_time = get_sys_time()/1000;
#ifdef WIN32
    intptr_t handle;
    _finddata_t findData;
    handle = _findfirst(plogctx->plog_dir, &findData);
    if(handle == -1)
    {
        printf("Filed to find first file!\n");
        return ;
    }
    
    do
    {
        if(strstr(findData.name, ".log"))
        {
            char path[256] = {0};
            sprintf(path, "%s/%s", plogctx->plog_dir, findData.name);
            int modtime = get_file_modify_time(path);
            if((cur_time - modtime)/3600 > hours)
            {
                remove(path);
            }
        }
    ...
    struct stat st;
    char p[256] = {0};
    const char* path = plogctx->plog_dir;
    ...
    while((dp = readdir(d)) != NULL) {
        ...
        if(!S_ISDIR(st.st_mode)) {
            //printf("%s\n", dp->d_name);
            int modtime = get_file_modify_time(p);
            if((cur_time - modtime)/3600 > hours)
            {
                remove(p);
            }
    ...
    struct stat st;
    char plogfile[256] = {0};
    if(stat(plogctx->plog_dir, &st) < 0 || !S_ISDIR(st.st_mode))
    {
        printf("Invalid path %s\n", plogctx->plog_dir);
        return ;
    }
    d = opendir(plogctx->plog_dir);
    if(!d)
    {
        printf("opendir[%s] error:%d, reason:%s\n", plogctx->plog_dir, errno, strerror(errno));
        return ;
    }
    ...
    while((dp == readdir(d)) != NULL)
    {
        ...
        snprintf(plogfile, sizeof(plogfile) - 1, "%s/%s", plogctx->plog_dir, dp->d_name);
        stat(plogfile, &st);
        if(!S_ISDIR(st.st_mode))
        {
            //log file
...
static log_ctx* open_log_contex(const int nlog_leve, int nlog_output_flag, const char* plog_path)
{
    ...
    plogctx->ngmtime	       = 1;
    printf("open_log_contex end, plog_path:%s, plogctx:%p\n", plog_path, plogctx);
    return plogctx;
}
...
static log_ctx* init_file_log(const char* plogdir, const int nloglevel, const int nlogoutputflag,  const int nmayjor, const int nminor, const int nmicro, const int ntiny)
{
    ...
    if(plogdir && (nlogoutputflag&LOG_OUTPUT_FLAG_FILE))
    {
        ...
        plog_dir = (char*)malloc(strlen(logpath)+1);
        strcpy(plog_dir, logpath);
        strcpy(logpath, plogdir);
        if(!strstr(logpath, ".log"))
        {
            struct tm* ptm;
            time_t rawtime;
            time(&rawtime);
            ...
            char datebuf[256] = {0};
            snprintf(datebuf, 256, "%04d%02d%02d-%02d%02d%02d.log", 1900 + ptm->tm_year, 1 + ptm->tm_mon, ptm->tm_mday, ptm->tm_hour, ptm->tm_min, ptm->tm_sec);
            strcat(logpath, datebuf);
        }
        /*else if(logpath[strlen(logpath)-1] != '/')
        {
            logpath[strlen(logpath)] = '/';
        }
        else
        {
            sprintf(&logpath[strlen(logpath)], "ijkplayer_%lu.log", get_sys_time());
        }*/
    ...
    plogctx->plog_dir = plog_dir;//(char*)malloc(strlen(plogdir)+1);
...
static int write_log(log_ctx* plogctx, const int nlevel, char* plogbuf, const int nloglen)
{
...
#else
            printf("\033[33m%.*s\033[0m", loglen, "%s", plogbuf);
#endif
        } else{
#ifdef __ANDROID__
    ...
    if((LOG_OUTPUT_FLAG_FILE & plogctx->nlog_output_flag) && NULL != plogctx->pfd)
    {
	    fwrite(plogbuf, 1, loglen, plogctx->pfd);
		//if (nlevel >= LOG_LEVEL_ERROR)
		{
			// this code can be block in some android mobile phone
			//fflush(plogctx->pfd);
		}
    }
...
#ifdef __ANDROID__
        __android_log_print(ANDROID_LOG_INFO, "lazytrace", "plogctx->pfd:%p = fopen(plogctx->plog_path:%s, wb)\n", plogctx->pfd, plogctx->plog_path);
#else
 ...
 * @return 0为成功，否则为失败
 **********************************************************************************************************/
static int log_memory(log_ctx* plogctx, const int nlevel, const char* tag, const char* pfile, int line, const char* pfun, const char* pmemory, int len, const char* format, ...)
{
    check_pointer(plogctx, -1);
    int ret = 0;
    lock_mutex(plogctx->pmutex);
    ...
    int nlog_buf_size = plogctx->nlog_buf_size;
    if (!generate_header(plogctx, plogctx->plog_buf, tag, pfile, line, pfun, nlevel, &nlog_buf_size))
    {
        unlock_mutex(plogctx->pmutex);
        return ret;
    ...
    va_start(ap, format);
    // we reserved 1 bytes for the new line.
    nlog_buf_size += vsnprintf(plogctx->plog_buf + nlog_buf_size, plogctx->nlog_buf_size - nlog_buf_size, format, ap);
    va_end(ap);
...
static const char* get_format_by_log_path(char* format, ...)
{
    static char path[1024];
    int offset = 0;
    if(!g_plogctx || NULL == format)
    {
        return NULL;
    }
    sprintf(path, "%s/", get_log_path());
    offset = strlen(path);
    va_list ap;
    va_start(ap, format);
    // we reserved 1 bytes for the new line.
    offset += vsnprintf(path + offset, 1024 - offset, format, ap);
    va_end(ap);
...
static const char* gen_url_by_log_path(const char* pname)
{
    static char url[1024];
    if(!g_plogctx || NULL == pname)
    {
        return NULL;
    }
    
    sprintf(url, "%s/%s", get_log_path(), pname);
...
static void ffmpeg_log_callback(void *ptr, int level, const char *fmt, va_list vl)
{
	...
	int offset = 0;
/*#ifdef WIN32
    sprintf_t(temp, 1024, "level:%d ", level);
#else
	sprintf(temp, 1024, "level:%d ", level);
#endif*/
	offset = strlen(temp);

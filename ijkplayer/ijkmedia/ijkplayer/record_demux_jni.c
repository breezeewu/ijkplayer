...
static jint MediaRecord_Open(JNIEnv *env, jobject thiz, /* jobject weakthiz,*/ jstring srcurl, jstring sinkurl, jstring tmpurl)
{
	...
	lbtrace("prc:%p = avrecord_open_context(psrcurl:%s, psinkurl:%s, mov, ptmpurl:%s)\n", prc, psrcurl, psinkurl, ptmpurl);
    if(NULL == prc)
    {
	...
	jobject weak_thiz = (*env)->NewGlobalRef(env, thiz);//(*env)->NewWeakGlobalRef(env, thiz);

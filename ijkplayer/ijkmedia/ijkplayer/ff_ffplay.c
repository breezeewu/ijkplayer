...
static int packet_queue_put_private(PacketQueue *q, AVPacket *pkt)
{
    ...
    {
        q->serial++;
        sv_trace("flush_queue q->serial:%d\n");
    }
    ...
    int64_t cur_pts = pkt1->pkt.pts == AV_NOPTS_VALUE ? pkt1->pkt.dts : pkt1->pkt.pts;
    if(q->last_pkt_pts != AV_NOPTS_VALUE && AV_NOPTS_VALUE != cur_pts)
    {
        q->duration += abs(cur_pts - q->last_pkt_pts);
    }
    else
    {
    ...
    /*int64_t curduration = q->last_pkt_pts == AV_NOPTS_VALUE ? MIN_PKT_DURATION
    q->duration += FFMAX(q->last_pkt_pts, MIN_PKT_DURATION);*/
    if(AV_NOPTS_VALUE != pkt1->pkt.pts || AV_NOPTS_VALUE != pkt1->pkt.dts)
    {
        q->last_pkt_pts = pkt1->pkt.pts == AV_NOPTS_VALUE ? pkt1->pkt.dts : pkt1->pkt.pts;
    }
    
    //q->last_pkt_pts = pkt1->pkt.pts;
...
static int64_t get_packet_queue_duration(PacketQueue *q)
{
    if(!q)
    {
        return -1;
    ...
    {
        int64_t last_pts = q->last_pkt->pkt.pts == AV_NOPTS_VALUE ? q->last_pkt->pkt.dts : q->last_pkt->pkt.pts;
        int64_t first_pts = q->first_pkt->pkt.pts  == AV_NOPTS_VALUE ? q->first_pkt->pkt.dts : q->first_pkt->pkt.pts;
    ...
    for (;;) {
            ...
            do {
                ...
                switch (d->avctx->codec_type) {
                    case AVMEDIA_TYPE_VIDEO:
                        ...
                        //sv_trace("video ret:%d, frame->pts:%"PRId64"", ret, frame->pts);
                        if (ret >= 0) {
                    ...
                    case AVMEDIA_TYPE_AUDIO:
                        ret = avcodec_receive_frame(d->avctx, frame);
                        //sv_trace("audio frame->pts:%"PRId64"", frame->pts);
                        if (ret >= 0) {
                            AVRational tb = (AVRational){1, frame->sample_rate};
                            if (frame->pts != AV_NOPTS_VALUE)
                            {
                                int64_t pts = frame->pts;
                                AVRational codectb = av_codec_get_pkt_timebase(d->avctx);
                                frame->pts = av_rescale_q(frame->pts, codectb, tb);
                                //static int64_t decoder_sum_dur = 0;
                                //decoder_sum_dur += frame->nb_samples * 1000 / frame->sample_rate;
                                //lbtrace("pts:%"PRId64", frame->pts:%"PRId64", codectb.den:%d, codectb.num:%d\n", pts, frame->pts, codectb.den, codectb.num);
                            }
        ...
        if (pkt.data == flush_pkt.data) {
            ...
            if (d->avctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {
                ...
                //sv_trace("idx:%d, video que size:%d, audio que size:%d  pkt->pts:%"PRId64"", pkt.stream_index, ffp->is->videoq.size, ffp->is->audioq.size, pkt.pts);
...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    SDL_LockMutex(ffp->is->play_mutex);
    if(NULL == is->pcur_frame ||
       vp->frame->width != is->pcur_frame->width ||
       vp->frame->height != is->pcur_frame->height ||
       vp->frame->format != is->pcur_frame->format ||
       is->pcur_frame->data[0] == NULL)
    {
        if(is->pcur_frame)
        {
            av_frame_free(&is->pcur_frame);
        }
        is->pcur_frame = av_frame_alloc();
        is->pcur_frame->width = vp->frame->width;
        is->pcur_frame->height = vp->frame->height;
        is->pcur_frame->format = vp->frame->format;
        //av_frame_copy_props(vp->frame, src_frame);
        av_frame_get_buffer(is->pcur_frame, 4);
    }
    av_frame_copy(is->pcur_frame, vp->frame);
    is->pcur_frame->pkt_pos = vp->pos;
    if(is->llvframe_count++%15 ==0)
    {
        sv_trace("video render pts:%lf, nbpkt:%d, pciqsize:%d，clocktime:%lf, extclk time:%lf, pkt_pos:%" PRId64 "\n", vp->pts, is->videoq.nb_packets, is->pictq.size, get_master_clock(ffp->is), get_clock(&is->extclk), ffp->is->pcur_frame->pkt_pos);
    }
    is->dlast_render_vpts = vp->pts;
    ...
    if (vp->bmp) {
        ...
        /*double curtime = get_master_clock(ffp->is);
        //lbtrace("(vp->pts:%lf - get_master_clock(ffp->is):%lf):%lf", vp->pts, curtime, vp->pts - curtime);
        int i = 0;
        while(vp->pts - 0.02 > get_master_clock(ffp->is) && !is->abort_request && i++<25)
        {
            SDL_Delay(20);
            if(NAN == curtime)
            {
                lbtrace("NAN == curtime break!");
                break;
            }
        }*/
        SDL_VoutDisplayYUVOverlay(ffp->vout, vp->bmp);
        ffp->lldisplay_time = av_gettime();
        is->dlast_render_vpts = vp->pts;
        //av_log(ffp, AV_LOG_DEBUG, "display time:%ld\n", ffp->lldisplay_time - ffp->llstart_time);
        ffp->stat.vfps = SDL_SpeedSamplerAdd(&ffp->vfps_sampler, FFP_SHOW_VFPS_FFPLAY, "vfps[ffplay]");
        if (!ffp->first_video_frame_rendered) {
            ffp->first_video_frame_rendered = 1;
            ffp_notify_msg1(ffp, FFP_MSG_VIDEO_RENDERING_START);
            lbtrace("ffp_notify_msg1(ffp, FFP_MSG_VIDEO_RENDERING_START)\n");
        }
...
{
    sv_info("stream_component_close begin, stream_index:%d\n", stream_index);
    VideoState *is = ffp->is;
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        //lbtrace("after decoder_destroy(&is->auddec)\n");
        swr_free(&is->swr_ctx);
    ...
    sv_info("stream_component_close end\n");
}

static void stream_close(FFPlayer *ffp)
{
    sv_info("stream_close begin");
    ...
    packet_queue_abort(&ffp->pktq);
    av_log(NULL, AV_LOG_DEBUG, "wait for read_tid\n");
    SDL_WaitThread(is->read_tid, NULL);
    sv_info("after SDL_WaitThread\n");
    /* close each stream */
    ...
    if(ffp->pdev_ctrl)
    {
        dev_ctrl_close_contextp(&ffp->pdev_ctrl);
    }
#endif
    av_free(is);
    ffp->is = NULL;
    int64_t dur = get_sys_time() - begin;
    lbtrace("stream_close spend time:%"PRId64"", dur);
}
...
static double get_clock(Clock *c)
{
    ...
    {
        //sv_trace("*c:%p->queue_serial:%d != c->serial:%d\n", c, *c->queue_serial, c->serial);
        return NAN;
    }
    if (c->paused) {
        return c->pts;
...
static void set_clock_at(Clock *c, double pts, int serial, double time)
{
    ...
    c->serial = serial;
    //sv_trace("c:%p->serial = serial:%d", c, serial);
}
...
static void stream_seek(VideoState *is, int64_t pos, int64_t rel, int seek_by_bytes)
{
    ...
    //{
        ...
        if (seek_by_bytes)
            is->seek_flags |= AVSEEK_FLAG_BYTE;
        is->seek_req = 1;
        sv_trace("seek(pos:%"PRId64", rel:%"PRId64")", pos, rel);
        SDL_CondSignal(is->continue_read_thread);
    //}
    /*else
    {
        sv_trace("is->seek_req = %d, drop seek request!", is->seek_req);
    }*/
}
...
static double compute_target_delay(FFPlayer *ffp, double delay, VideoState *is)
{
    ...
    //lbtrace("clocktype:%d, diff:%lf = get_clock(&is->vidclk):%lf - get_master_clock(is):%lf, delay:%lf, sync_threshold:%lf\n", get_master_sync_type(is), diff, get_clock(&is->vidclk), get_master_clock(is), delay, sync_threshold);
    if (ffp) {
...
static void video_refresh(FFPlayer *opaque, double *remaining_time)
{
    ...
    if (!ffp->display_disable && is->show_mode != SHOW_MODE_VIDEO && is->audio_st) {
        ...
        *remaining_time = FFMIN(*remaining_time, is->last_vis_time + ffp->rdftspeed - time);
        lbtrace("*remaining_time:%lf = FFMIN(*remaining_time, is->last_vis_time:%lf + ffp->rdftspeed:%lf - time:%lf)\n", *remaining_time, is->last_vis_time, ffp->rdftspeed, time);
    }
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            time= av_gettime_relative()/1000000.0;
            // times up, display immediately
            if(vp->pts < get_master_clock(is) && !isnan(get_master_clock(is)))
            {
                is->force_refresh = 1;
                frame_queue_next(&is->pictq);
                goto display;
            ...
            if (time < is->frame_timer + delay) {
                *remaining_time = FFMIN(is->frame_timer + delay - time, *remaining_time);
                //lbtrace("*remaining_time:%lf = FFMIN((is->frame_timer:%lf + delay:%lf - time:%lf):%lf, *remaining_time)，last_duration:%lf\n", *remaining_time, is->frame_timer, delay, time, (is->frame_timer + delay - time), last_duration);
                goto display;
            }

            /*if(vp->pts > get_master_clock(is) + REFRESH_RATE*2)
            {
                if(vp->pts - get_master_clock(is) > 1.0 && !vp->pts_unusual)
                {
                    lbtrace("pts unusual, (vp->pts:%lf - get_master_clock(is):%lf):%lf\n", vp->pts, get_master_clock(is), vp->pts - get_master_clock(is));
                    vp->pts_unusual = 1;
                }
                goto display;
            }*/
...
display:
        ...
        {
            //lbtrace("ffp->display_disable:%d && is->force_refresh:%d && is->show_mode:%d == SHOW_MODE_VIDEO:%d && is->pictq.rindex_shown:%d\n", ffp->display_disable, is->force_refresh, is->show_mode, SHOW_MODE_VIDEO, is->pictq.rindex_shown);
            video_display2(ffp);
        }
        /*else
        {
...
//#ifdef FFP_MERGE
           ...
           vp->frame->data[0] == NULL)
        {
            ...
            vp->frame->format = src_frame->format;
            //av_frame_copy_props(vp->frame, src_frame);
            av_frame_get_buffer(vp->frame, 4);
        }
        av_frame_copy(vp->frame, src_frame);
        SDL_UnlockMutex(ffp->is->play_mutex);
//#endif
...
reload:
...
#if 1
        // drop frame condition
        if(is->blive && is->audioq.nb_packets + is->sampq.size > 10)
        {
            is->bdrop_audio = true;
            sv_trace("is->blive:%d && is->audioq.nb_packets:%d + is->sampq.size:%d > 10, drop audio begin\n", is->blive, is->audioq.nb_packets, is->sampq.size);
        };
        
        while(is->bdrop_audio)
        {
            af = frame_queue_peek_readable(&is->sampq);
            sv_trace("drop audio frame pts:%lf, is->audioq.nb_packets:%d, is->sampq.size:%d, is->llaframe_couont:%"PRId64"", af->pts, is->audioq.nb_packets, is->sampq.size, is->llaframe_count++);
            frame_queue_next(&is->sampq);
            if(is->audioq.nb_packets + is->sampq.size <= 2)
            {
                is->bdrop_audio = false;
                break;
            }
            else if(is->sampq.size <= 0)
            {
                break;
            }
        }
#else
#ifdef ENABLE_LOG_AUDIO_DATA
        while(/*is->blive*/false &&is->sampq.size > 2 && (is->audioq.nb_packets + is->sampq.size > 5) && is->llaframe_couont++%7 == 0)
#else
        while(is->blive &&is->sampq.size > 2 && (is->audioq.nb_packets + is->sampq.size > 5) && is->llaframe_couont++%7 == 0)
#endif
        
        {
            af = frame_queue_peek_readable(&is->sampq);
            sv_trace("drop audio frame pts:%lf, is->audioq.nb_packets:%d, is->sampq.size:%d, is->llaframe_couont:%"PRId64"", af->pts, is->audioq.nb_packets, is->sampq.size, is->llaframe_couont);
            frame_queue_next(&is->sampq);
        };
#endif
    ...
    //lbtrace("wait for audio packet spend time:%" PRId64"", av_gettime() - begin);
    ...
    static int64_t sumbytes = 0, audiotime = 0;
    sumbytes += resampled_data_size;
    audiotime = sumbytes * 1000/is->audio_tgt.bytes_per_sec;
    //static double last_pts = 0;
    is->llaframe_count++;
    //if((is->llaframe_count++%15)==0)
    {
        sv_trace("audio render pts:%lf, nbpacket:%d, samsize:%d, curpos:%ld, audiotime:%"PRId64", is->av_sync_type:%d, size:%d, is->llaframe_count:%" PRId64 "\n", af->pts, is->audioq.nb_packets, is->sampq.size, ffp_get_current_position_l(ffp), audiotime, is->av_sync_type, af->frame->linesize[0], is->llaframe_count);
        //is->llaframe_couont++;
        //sv_trace("audio render frame pts:%lf, curpos:%ld, sumbytes:%"PRId64", audiotime:%"PRId64", audio duration:%"PRId64"", af->pts, ffp_get_current_position_l(ffp), sumbytes, audiotime, af->pts - last_pts);
    }
    is->dlast_render_apts = af->pts;
    audio_clock0 = is->audio_clock;
    /* update the audio clock with the pts */
    if (!isnan(af->pts))
    {
        is->audio_clock = af->pts + (double) af->frame->nb_samples / af->frame->sample_rate;
        is->llast_render_atime = av_gettime();
    }
    else
        is->audio_clock = NAN;
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    FFPlayer *ffp = opaque;
    VideoState *is = ffp->is;
    Uint8* prend_pcm_buf = stream;
    int buf_len = len;
    int audio_pcm_size = 0;
    memset(stream, 0, len);
    ffp->laudio_render_begin = av_gettime();
    //unsigned long begin = get_sys_time();
    ...
    while (len > 0) {
        ...
        if (!is->muted && is->audio_buf && is->audio_volume == SDL_MIX_MAXVOLUME)
        {
            ...
            is->llaframe_count++;
                sv_trace("audio render pts:%lf, nbpacket:%d, samsize:%d, curpos:%ld, is->av_sync_type:%d, is->llaframe_count:%" PRId64 ", muted:%d, audio_pcm_size:%d, is->buffering_on:%d\n", is->dlast_render_apts, is->audioq.nb_packets, is->sampq.size, ffp_get_current_position_l(ffp), is->av_sync_type, is->llaframe_count, is->muted, audio_pcm_size, is->buffering_on);
        }
        else {
            ...
            sv_trace("audio render mix audio, pts:%lf, nbpacket:%d, sampsize:%d, curpos:%ld, av_sync_type:%d, is->llaframe_count:%" PRId64 ", muted:%d, audio_pcm_size:%d, is->buffering_on:%d\n", is->dlast_render_apts, is->audioq.nb_packets, is->sampq.size, ffp_get_current_position_l(ffp), is->av_sync_type, is->llaframe_count, is->muted, audio_pcm_size, is->buffering_on);
            }
            else
            {
                sv_trace("audio render muted audio, pts:%lf, nbpacket:%d, sampsize:%d, curpos:%ld, av_sync_type:%d, is->llaframe_count:%" PRId64 ", muted:%d, audio_pcm_size:%d, is->buffering_on:%d\n", is->dlast_render_apts, is->audioq.nb_packets, is->sampq.size, ffp_get_current_position_l(ffp), is->av_sync_type, is->llaframe_count, is->muted, audio_pcm_size, is->buffering_on);
            }
        }
    ...
    if((!ffp->packet_buffering || is->auddec.finished) && audio_pcm_size <= 0)
    //if(!ffp->packet_buffering && audio_pcm_size <= 0)
    {
        double offset = (av_gettime() - is->llast_render_atime)/1000000.0;
        sv_trace("is->audio_clock:%lf = is->audio_clock:%lf + offset:%lf\n", is->audio_clock + offset, is->audio_clock, offset);
        is->audio_clock = is->audio_clock + offset;
        //double audio_clock_time = is->audio_clock + (av_gettime() - is->llast_render_atime)/1000000.0;
        //is->audio_clock = FFMIN(audio_clock_time, get_clock(&is->extclk) + 0.5);
        //sv_trace("is->audio_clock:%lf = FFMIN(audio_clock_time:%lf, get_clock(&is->extclk):%lf)\n", is->audio_clock, audio_clock_time, get_clock(&is->extclk)+0.5);
        is->llast_render_atime = av_gettime();
        
        //is->audio_clock += (double)(is->audio_buf_index) / is->audio_tgt.bytes_per_sec;
    }
    /* Let's assume the audio driver that is used by SDL has two periods. */
    if (!isnan(is->audio_clock)) {
        set_clock_at(&is->audclk, is->audio_clock - (double)(is->audio_write_buf_size) / is->audio_tgt.bytes_per_sec - SDL_AoutGetLatencySeconds(ffp->aout), is->audio_clock_serial, ffp->audio_callback_time / 1000000.0);
        sv_trace("audio pts:%lf, pcm duration:%lf\n", is->audio_clock, (double)(is->audio_write_buf_size) / is->audio_tgt.bytes_per_sec - SDL_AoutGetLatencySeconds(ffp->aout));
        //sv_trace("sdl_audio_callback set_clock, is->audio_clock:%lf, is->audio_buf_index:%d, is->audio_tgt.bytes_per_sec:%d,SDL_AoutGetLatencySeconds(ffp->aout):%lf = %lf\n", is->audio_clock, is->audio_buf_index, is->audio_tgt.bytes_per_sec, SDL_AoutGetLatencySeconds(ffp->aout), is->audio_clock - (double)(/*is->audio_write_buf_size*/is->audio_buf_index) / is->audio_tgt.bytes_per_sec - SDL_AoutGetLatencySeconds(ffp->aout));
    ...
    ffp->laudio_render_end = av_gettime();
    /*if(ffp->laudio_render_end - ffp->laudio_render_begin > 50)
    {
        sv_trace("sdl_audio_callback end, spend time:%lu, nb_packets:%d, is->muted:%d, is->audio_volume:%d, len:%d\n", ffp->laudio_render_end - ffp->laudio_render_begin, ffp->is->audioq.nb_packets, (int)is->muted, is->audio_volume, len);
    }*/

#ifdef ENABLE_LOG_AUDIO_DATA
    if(ffp->paudio_pcm_file && buf_len > 0 && prend_pcm_buf)
    {
        int write_len = fwrite(prend_pcm_buf, 1, buf_len, ffp->paudio_pcm_file);
        sv_trace("write_len:%d = fwrite(prend_pcm_buf:%p, 1, buf_len:%d, ffp->paudio_pcm_file:%p), pos:%ld\n", write_len,  prend_pcm_buf, buf_len, ffp->paudio_pcm_file, ftell(ffp->paudio_pcm_file));
    }
#endif
...
static int audio_open(FFPlayer *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    double delay_sec = ((double)(2 * spec.size)) / audio_hw_params->bytes_per_sec;
    SDL_AoutSetDefaultLatencySeconds(ffp->aout, delay_sec);
    sv_trace("SDL_AoutSetDefaultLatencySeconds(ffp->aout:%p, delaysec:%lf), spec.size:%d, audio_hw_params->bytes_per_sec:%d\n", ffp->aout, delay_sec, spec.size, audio_hw_params->bytes_per_sec);
    return spec.size;
}
...
bool need_buffering(FFPlayer *ffp)
{
    VideoState *is = ffp->is;
    if(!is->blive && ffp->packet_buffering && !is->buffering_on)
    {
        ...
        else if(is->audio_st && is->audioq.nb_packets <= 0)
        {
            //ffp_toggle_buffering(ffp, 1);
            return true;
        }
...
static int packet_thread(void *arg)
{
    int ret = 0;
    int apkt_num = 0;
    FFPlayer *ffp = arg;
    ...
    AVRational std_timebase = {1, AV_TIME_BASE};
    enum AVCodecID vcodecid = AV_CODEC_ID_NONE;
    //vcodecid = AV_CODEC_ID_HEVC; AV_CODEC_ID_H264;
    enum AVCodecID acodecid = AV_CODEC_ID_NONE;
    //bool bwait_for_keyframe = true;
    //acodecid = AV_CODEC_ID_AAC;
    //int veof = 0;
    //int aeof = 0;
    lbsc_xvc_ctx* pxc = NULL;// lbsc_open_xvc_context(int stream_type);
    int complete = 0;
    ///int mt = -1;
    int codec_id = -1;
    VAVA_HS_MUX_CTX* pmc = NULL;
    char* prec_log_path = NULL;
    if(g_pconf_ctx)
    {
        const char* prec_path = get_conf_value_by_tag(g_pconf_ctx, "log_player_deliver_rec");
        lbtrace("prec_path:%s = get_conf_value_by_tag(g_pconf_ctx:%p, log_player_deliver_rec)", prec_path, g_pconf_ctx);
        if(prec_log_path)
        {
            int len = strlen(get_log_path()) + strlen(prec_log_path) + 2;
            prec_log_path = (char*)malloc(len);
            sprintf(prec_log_path, "%s/%s", get_log_path(), prec_log_path);
            lbtrace("sprintf(prec_log_path:%s, get_log_path():%s, prec_path:%s)\n", prec_log_path, get_log_path(), prec_path);
        }
        //vava_hs_create_mux_record(prec_log_path, 4, 1, int vcodec_id, int acodec_id, int fps, int samplerate, int encrypt)
    }
    while(1)
    {
        if (is->abort_request)
        ...
        if((is->bdevlier_veof && is->bdevlier_aeof) && (!is->audio_st || (is->auddec.finished == is->audioq.serial && frame_queue_nb_remaining(&is->sampq) == 0)) &&
         (!is->video_st || (is->viddec.finished == is->videoq.serial && frame_queue_nb_remaining(&is->pictq) == 0)))
        {
            ffp_notify_msg1(ffp, FFP_MSG_COMPLETED);
            sv_trace("ffp_notify_msg1(ffp, FFP_MSG_COMPLETED)\n");
            complete = 1;
            ffp_pause_l(ffp);
            continue;
        }
        if(is->bdevlier_veof && is->bdevlier_aeof && is->buffering_on)
        {
        ...
        AVPacket pkt;
        ret = packet_queue_get(&ffp->pktq, &pkt, 0, 0);
        if(ret <= 0)
        {
            if(need_buffering(ffp))
            {
                ffp_toggle_buffering(ffp, 1);
            }
			SDL_Delay(10);
        ...
        codec_id = pkt.stream_index;
        pkt.stream_index = get_media_type_from_codec_id(codec_id);
        
        if(AVMEDIA_TYPE_VIDEO == pkt.stream_index)// AV_CODEC_ID_H264 == pkt.stream_index || AV_CODEC_ID_HEVC == pkt.stream_index)
        {
            if(AV_CODEC_ID_NONE != vcodecid && vcodecid != codec_id)
            {
                lberror("video codec change, vcodecid from %d to %d,break\n", vcodecid, pkt.stream_index);
                break;
            }
            vcodecid = codec_id;
            if(NULL == pxc)
            {
                pxc = lbsc_open_xvc_context(AV_CODEC_ID_H264 == vcodecid ? 4 : 5);
            }
            ...
            {
                // create record file
                if(prec_log_path && !pmc)
                {
                    pmc = vava_hs_create_mux_record(prec_log_path, 4, 1, vcodecid, AV_CODEC_ID_NONE == acodecid ? AV_CODEC_ID_AAC : acodecid, 15, 16000, 0);
                }
                AVCodec* pcodec = NULL;
                char* pextradata = NULL;
                ret = parse_stream(pxc, (char*)pkt.data, pkt.size);
                //ret = is_idr_frame(AV_CODEC_ID_H264 == vcodecid ? 4 : 5, (const char*)pkt.data, pkt.size);
                if(pxc->m_nidr_frame)
                {
                    int size = 0;
                    pcodec = avcodec_find_decoder(pxc->m_nstream_type);
                    AVStream* pst = avformat_new_stream(is->ic, pcodec);
                    pst->index = is->ic->nb_streams - 1;
                    pst->time_base.den = 1000000;
                    pst->time_base.num = 1;
                    pst->r_frame_rate.den = 1;
                    pst->r_frame_rate.num = 25;
                    pst->discard = AVDISCARD_NONE;
                    pst->codecpar->width = pxc->m_nwidth;
                    pst->codecpar->height = pxc->m_nheight;
                    pst->codecpar->codec_type = AVMEDIA_TYPE_VIDEO;
                    pst->codecpar->codec_id = vcodecid;
                    pst->codecpar->bit_rate = pxc->m_nheight/2000;
                    if(size > 0 && pextradata)
                    {
                        pst->codecpar->extradata = (uint8_t*)malloc(size);
                        memcpy(pst->codecpar->extradata, pextradata, size);
                        pst->codecpar->extradata_size = size;
                    }
                    is->viddec.start_pts = pkt.pts;
                    is->viddec.start_pts_tb = pst->time_base;
                    if(ffp->videotoolbox)
                    {
                        pst->codecpar->format = AV_PIX_FMT_NV12;
                    }
                    else
                    {
                        pst->codecpar->format = AV_PIX_FMT_YUV420P;
                    }
                    //sps_context_close(&pspsctx);
                    ret = stream_component_open(ffp, pst->index);
                    if(ret < 0)
                    {
                        av_log(ffp, AV_LOG_ERROR, "stream_component_open video stream failed, ret:%d\n", ret);
                        break;
                    }
                    is->video_stream = pst->index;
                }
                else
                {
                    av_packet_unref(&pkt);
            ...
            //sv_info("video pkt.pts:%"PRId64" = pkt.pts:%"PRId64" - start_time:%"PRId64"\n", pkt.pts - start_time, pkt.pts, start_time);
            pkt.pos = pkt.pts;
#ifdef FIX_DELIVER_PACKET_TIMESTAMP
            if(ffp->pdev_ctrl)
            {
                pkt.dts = pkt.pts = fix_timestamp(ffp->pdev_ctrl, codec_id, pkt.pts);
            }
#else
            pkt.pts -= start_time;
#endif
            sv_trace("video codec_id:%d, pkt.pos:%" PRId64 ", pkt.pts:%" PRId64 ", pkt.size:%d\n", codec_id, pkt.pos, pkt.pts, pkt.size);
            packet_queue_put(&ffp->is->videoq, &pkt);
            ffp_statistic_l(ffp);
            ffp_check_buffering_l(ffp);
            int64_t dur = get_packet_queue_duration(&ffp->is->videoq);
            if(!bvideochange && dur > 1000000)
            {
                AVStream* pst = is->ic->streams[pkt.stream_index];
                ffp_notify_msg1(ffp, FFP_MSG_COMPONENT_OPEN);
                ffp_notify_msg3(ffp, FFP_MSG_VIDEO_SIZE_CHANGED, pst->codecpar->width, pst->codecpar->height);
                ffp_notify_msg3(ffp, FFP_MSG_SAR_CHANGED, pst->codecpar->sample_aspect_ratio.num,pst->codecpar->sample_aspect_ratio.den);
                bvideochange = true;
                ffp_toggle_buffering(ffp, 0);
            }
        }
        ...
        {
            if(AV_CODEC_ID_NONE != acodecid)
            {
                acodecid = codec_id;//pkt.stream_index;
            }
            apkt_num++;
            ...
            if(apkt_num >= 3 && !ffp->is->audio_st && pkt.data[0] == 0xff && pkt.size > 7)
            {
                adts_ctx* padtsctx = adts_demux_open((char*)pkt.data, pkt.size);
                char seq[2] = {0};
                int seq_len = mux_sequence_header(padtsctx, seq, 2);
                if(seq_len <= 0)
                {
                    av_log(ffp, AV_LOG_ERROR, "mux_sequence_header failed, ret:%d\n", ret);
                }
                AVCodec* pcodec = avcodec_find_decoder(acodecid);
                AVStream* pst = avformat_new_stream(is->ic, pcodec);
                pst->index = is->ic->nb_streams - 1;
                pst->time_base = std_timebase;//(AVRational){get_samplerate(padtsctx), 1};
                //pst->time_base.den = get_samplerate(padtsctx);
                //pst->time_base.num = 1;
                pst->discard = AVDISCARD_NONE;
                pst->codecpar->channels = padtsctx->channel_configuration;
                pst->codecpar->sample_rate = get_samplerate(padtsctx);
                pst->codecpar->codec_type = AVMEDIA_TYPE_AUDIO;
                pst->codecpar->codec_id = AV_CODEC_ID_AAC;
                pst->codecpar->bit_rate = 128;
                if(seq_len > 0)
                {
                    pst->codecpar->extradata = (uint8_t*)malloc(seq_len);
                    memcpy(pst->codecpar->extradata, seq, seq_len);
                    pst->codecpar->extradata_size = seq_len;
                }
                
                pst->codecpar->format = AV_SAMPLE_FMT_S32;
                pst->codecpar->channel_layout = av_get_default_channel_layout(padtsctx->channel_configuration);
                is->auddec.start_pts = pkt.pts;
                is->auddec.start_pts_tb = pst->time_base;
                ret = stream_component_open(ffp, pst->index);
                //sv_trace("is->audio_st->time_base.den:%d, is->audio_st->time_base.num:%d", is->audio_st->time_base.den, is->audio_st->time_base.num);
                if(ret < 0)
                {
                    av_log(ffp, AV_LOG_ERROR, "stream_component_open audio stream failed, ret:%d\n", ret);
                    break;
                }
                is->audio_stream = pst->index;
                if(AV_NOPTS_VALUE == start_time)
                {
                    start_time = pkt.pts;
                }
                if(ffp->is->auddec.avctx)
                {
                    ffp->is->auddec.avctx->time_base.den = 1;
                    ffp->is->auddec.avctx->time_base.num = 1000000;
                }
                if(bvideochange)
                {
                    ffp_notify_msg1(ffp, FFP_MSG_COMPONENT_OPEN);
            ...
            pkt.duration = 10000;
            //sv_info("audio pkt.pts:%"PRId64" = pkt.pts:%"PRId64" - start_time:%"PRId64"\n", pkt.pts - start_time, pkt.pts, start_time);
            //pkt.pts -= start_time;
            pkt.pos = pkt.pts;
#ifdef FIX_DELIVER_PACKET_TIMESTAMP
            if(ffp->pdev_ctrl)
            ...
            pkt.pts -= start_time;
#endif
            sv_trace("audio codec_id:%d, pkt.pos:%" PRId64 ", pkt.pts:%" PRId64 ", pkt.size:%d\n", codec_id, pkt.pos, pkt.pts, pkt.size);
            int64_t oldpts = pkt.pts;
            pkt.pts = av_rescale_q(pkt.pts, std_timebase, std_timebase/*is->audio_st->time_base*/);
            pkt.duration = av_rescale_q(pkt.duration, std_timebase, std_timebase/*is->audio_st->time_base*/);
            packet_queue_put(&ffp->is->audioq, &pkt);
            ffp_statistic_l(ffp);
            ffp_check_buffering_l(ffp);
            /*static int aframecount = 0;
            if(0 == aframecount++%15)
            {
                lbtrace("audio queue put pakcet, ffp->is->audioq.nb_packets:%d, ffp->is->audioq.size:%d, pkt.pts:%"PRId64", oldpts:%"PRId64"\n", ffp->is->audioq.nb_packets, ffp->is->audioq.size, pkt.pts, oldpts);
            }*/
        }
        else if(AVMEDIA_TYPE_SUBTITLE == pkt.stream_index)
        {
            packet_queue_put(&ffp->is->subtitleq, &pkt);
        }
        if((ffp->is->audio_st || is->video_st) && !ffp->prepared)
        {
            ...
            {
                ffp_toggle_buffering(ffp, 0);
                sv_trace("live ffp_toggle_buffering(ffp, 0)\n");
            }
            else
            {
                if (is->audio_stream >= 0) {
        ...
        if(pmc)
        {
            int keyflag = codec_id == AV_CODEC_ID_AAC ? 1 : 0;
            ret = vava_hs_write_mux_record(pmc, codec_id, pkt.data, pkt.size, keyflag, pkt.pos);
            lbtrace("ret:%d = vava_hs_write_mux_record(pmc:%p, codec_id:%d, pkt.data:%p, pkt.size:%d, keyflag:%d, pkt.pos:%" PRId64 ")\n", ret, pmc, codec_id, pkt.data, pkt.size, keyflag, pkt.pos);
        }
    }
    
    if(pmc)
    {
        ret = vava_hs_close_mux_record(&pmc);
        lbtrace("ret:%d = vava_hs_close_mux_record(&pmc:%p)\n", ret, pmc);
    }
...
{
    FFPlayer *ffp = arg;
    av_log(ffp, AV_LOG_DEBUG, "read_thread begin time:%ld\n", av_gettime() - ffp->llstart_time);
    VideoState *is = ffp->is;
    ...
    int init_ijkmeta = 0;
    //av_log(ffp, AV_LOG_DEBUG, "read_thread begin2 time:%ld\n", av_gettime() - ffp->llstart_time);
    if (!wait_mutex) {
        av_log(NULL, AV_LOG_FATAL, "SDL_CreateMutex(): %s\n", SDL_GetError());
        ret = AVERROR(ENOMEM);
        goto fail;
    ...
    is->eof = 0;
    //av_log(ffp, AV_LOG_DEBUG, "read_thread begin3 time:%ld\n", av_gettime() - ffp->llstart_time);
    ...
    //av_log(ffp, AV_LOG_DEBUG, "read_thread begin4 time:%ld\n", av_gettime() - ffp->llstart_time);
    ...
    //av_log(ffp, AV_LOG_DEBUG, "read_thread begin5 time:%ld\n", av_gettime() - ffp->llstart_time);
    ...
    //av_log(ffp, AV_LOG_DEBUG, "before open input time:%ld\n", av_gettime() - ffp->llstart_time);
    if (ffp->iformat_name)
    ...
    ffp->llopen_input_time = av_gettime();
    //av_log(ffp, AV_LOG_DEBUG, "open input time:%ld\n", ffp->llopen_input_time - ffp->llstart_time);
    ...
    if (ffp->find_stream_info) {
        ...
        do {
            ...
            ffp->llfind_stream_info_time = av_gettime();
            //av_log(ffp, AV_LOG_DEBUG, "find stream info time:%ld\n", ffp->llfind_stream_info_time - ffp->llstart_time);
        } while(0);
    ...
    if (ffp->start_time != AV_NOPTS_VALUE) {
        ...
        //sv_trace("ret:%d = avformat_seek_file(ic, -1, INT64_MIN, timestamp:%"PRId64", INT64_MAX, 0)", ret, timestamp);
        if (ret < 0) {
...
#endif
        if (is->seek_req) {
            //pthread_mutex_lock(is->play_mutex);
            is->seek_req = 0;
...
//      of the seek_pos/seek_rel variables
            ...
            //sv_trace("before avformat_seek_file, seek_target:%"PRId64"", seek_target);
            ret = avformat_seek_file(is->ic, -1, seek_min, seek_target, seek_max, is->seek_flags);
            sv_trace("ret:%d = avformat_seek_file(is->ic, -1, seek_min, seek_target:%"PRId64", seek_max, is->seek_flags), is->seek_pos:%"PRId64"\n", ret, seek_target, is->seek_pos);
            if (ret < 0) {
                av_log(NULL, AV_LOG_ERROR,
                ...
                if (is->audio_stream >= 0) {
                    ...
                    //sv_trace("seek packet_queue_put(&is->audioq, &flush_pkt)\n");
                ...
                if (is->subtitle_stream >= 0) {
                    ...
                    //sv_trace("seek packet_queue_put(&is->subtitleq, &flush_pkt)\n");
                }
                if (is->video_stream >= 0) {
                    if (ffp->node_vdec) {
                    ...
                    //sv_trace("seek packet_queue_put(&is->videoq, &flush_pkt)\n");
                }
        ...
        if (pkt->flags & AV_PKT_FLAG_DISCONTINUITY) {
            if (is->audio_stream >= 0) {
                packet_queue_put(&is->audioq, &flush_pkt);
                sv_trace("discontinue packet_queue_put(&is->audioq, &flush_pkt)\n");
            }
            if (is->subtitle_stream >= 0) {
                packet_queue_put(&is->subtitleq, &flush_pkt);
            }
            if (is->video_stream >= 0) {
                packet_queue_put(&is->videoq, &flush_pkt);
                sv_trace("discontinue packet_queue_put(&is->videoq, &flush_pkt)\n");
            }
        }
        ...
        tb.den = 1000000;
        tb.num = 1;
        int64_t pts = av_rescale_q(pkt_ts, ic->streams[pkt->stream_index]->time_base, tb);
        sv_trace("pkt->stream_index:%d, pkt->size:%d, pkt->pts:%"PRId64"\n", pkt->stream_index, pkt->size, pts);
        ...
        /*if(is->buffering_on && (is->videoq.nb_packets > 10 || is->video_stream < 0)&& (is->audioq.nb_packets > 10 || is->audio_stream < 0))
        {
            ffp_toggle_buffering(ffp, 0);
        }*/
        if (ffp->packet_buffering) {
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    {
        av_log(ffp, AV_LOG_DEBUG, "before read thread time:%ld\n", av_gettime() - ffp->llstart_time);
    ...
    {
        is->read_tid = SDL_CreateThreadEx(&is->_read_tid, packet_thread, ffp, "ff_packet");
        if (!is->read_tid) {
            av_log(NULL, AV_LOG_FATAL, "SDL_CreateThread(): %s\n", SDL_GetError());
            goto fail;
        }
...
static int video_refresh_thread(void *arg)
{
    ...
    while (!is->abort_request) {
        if (remaining_time > 0.0)
        {
            ...
            //lbtrace("av_usleep((int)(int64_t)(%d))", (int)(int64_t)(remaining_time * 1000000.0));
        }
...
{
    assert(ffp);
    av_log(ffp, AV_LOG_DEBUG, "prepare begin time:%ld\n", av_gettime() - ffp->llstart_time);
    //assert(file_name);
    ...
    packet_queue_init(&ffp->pktq);
    packet_queue_start(&ffp->pktq);
    ...
    if (!ffp->aout) {
        av_log(ffp, AV_LOG_DEBUG, "prepare befor ffpipeline_open_audio_output time:%ld\n", av_gettime() - ffp->llstart_time);
        ffp->aout = ffpipeline_open_audio_output(ffp->pipeline, ffp);
        av_log(ffp, AV_LOG_DEBUG, "prepare after ffpipeline_open_audio_output time:%ld\n", av_gettime() - ffp->llstart_time);
        if (!ffp->aout)
            return -1;
    }
    ...
    ffp->llprepare_time = av_gettime();
    /*if(NULL == ffp->input_filename)
    {
        ffp->is->blive = 1;
    }
    else
    ...
    av_log(ffp, AV_LOG_DEBUG, "prepare time:%ld\n", ffp->llprepare_time - ffp->llstart_time);
    return 0;
}
...
int ffp_wait_stop_l(FFPlayer *ffp)
{
    ...
    if (ffp->is) {
        ffp_stop_l(ffp);
        lbtrace("current render vpts:%lf, apts:%lf\n", ffp->is->dlast_render_vpts, ffp->is->dlast_render_apts);
        stream_close(ffp);
        ffp->is = NULL;
    }
    return 0;
...
{
    sv_info("ffp:%p, buffering_on:%d\n", ffp, buffering_on);
    if (!ffp->packet_buffering)
        return;
    
    VideoState *is = ffp->is;
    if (buffering_on && !is->buffering_on) {
        sv_trace("FFP_MSG_BUFFERING_START\n");
        ...
        sv_trace("FFP_MSG_BUFFERING_END\n");
        is->buffering_on = 0;
        stream_update_pause_l(ffp);
...
void ffp_track_statistic_l(FFPlayer *ffp, AVStream *st, PacketQueue *q, FFTrackCacheStatistic *cache)
{
    ...
    if (q && st && st->time_base.den > 0 && st->time_base.num > 0) {
        if( q->nb_packets > 0 && q->last_pkt_pts != AV_NOPTS_VALUE)
        {
            int64_t first_pkt_pts = 0;
            MyAVPacketList* cur_pkt = q->first_pkt;
            while(cur_pkt && AV_NOPTS_VALUE == cur_pkt->pkt.pts) cur_pkt = cur_pkt->next;
            if(cur_pkt)
            {
                first_pkt_pts = cur_pkt->pkt.pts;
            }
            else
            {
                first_pkt_pts = q->last_pkt_pts;
            }
            cache->duration = abs(q->last_pkt_pts - first_pkt_pts) * av_q2d(st->time_base) * 1000;;
        }
        else
        {
            cache->duration = 0;//q->duration * av_q2d(st->time_base) * 1000;
        }
        //cache->duration = get_packet_queue_duration(q) * av_q2d(st->time_base) * 1000;
...
void ffp_check_buffering_l(FFPlayer *ffp)
{
    ...
    if (hwm_in_ms > 0) {
        ...
        /*sv_trace("video_cached_duration:%"PRId64", audio_cached_duration:%"PRId64", time_per:%d, vpkt count:%d, apkt count:%d\n", video_cached_duration, audio_cached_duration, buf_time_percent, is->videoq.nb_packets, is->audioq.nb_packets);*/
    }
...
#endif
        //sv_trace("cached_size:%d, buf_size_percent:%d\n", cached_size, buf_size_percent);
    }
...
int ffp_set_playback_mute(FFPlayer *ffp, int muted)
{
    if (!ffp || !ffp->is)
        return -1;

    return ffp->is->muted = muted;
}

int ffp_get_playback_mute(FFPlayer *ffp)
{
    if (!ffp || !ffp->is)
        return -1;
...
#ifdef IJKPLAYER_SUPPORT_HEVC_RAW_DATA
int ffp_deliver_packet(FFPlayer *ffp, int codecid, char* pdata, int data_len, int64_t pts, int framenum)
{
    AVPacket pkt;
    if(!ffp || !ffp->is)
    {
        av_log(ffp, AV_LOG_ERROR, "ffp:%p, ffp->is:%p\n", ffp, ffp ? ffp->is : NULL);
        return -1;
    }
    ...
    {
        lbtrace("mt:%d deliver end of stream\n", codecid);
    }
#ifdef ENABLE_WAITING_FOR_KEYFRAME
    if(NULL == ffp->pdev_ctrl)
    {
        ffp->pdev_ctrl = dev_ctrl_open_context();
    }
    bool res = is_packet_dropable(ffp->pdev_ctrl, codecid, pdata, data_len, pts, framenum);
    //lbtrace("res:%d = is_packet_dropable(ffp->pdev_ctrl:%p, codecid:%d, pdata:%p, data_len:%d, pts:%" PRId64 ", framenum:%d)\n", res, ffp->pdev_ctrl, codecid, pdata, data_len, pts, framenum);
    if(ffp->pdev_ctrl && res)
    {
        lbtrace("wait for key frame, drop packet codec_id:%d, pdata:%p, len:%d, pts:%" PRId64 ", framenum:%ld\n", codecid, pdata, data_len, pts, framenum);
        return 0;
    }
#endif
    ...
    pkt.size = data_len;
    pkt.pos = pkt.pts = pts;
    pkt.stream_index = codecid;
    packet_queue_put(&ffp->pktq, &pkt);
    return 0;
}
#else
int ffp_deliver_packet(FFPlayer *ffp, int mediatype, char* pdata, int data_len, int64_t pts, int framenum)
{
    if(!ffp || !ffp->is)
    {
        av_log(ffp, AV_LOG_ERROR, "ffp:%p, ffp->is:%p\n", ffp, ffp ? ffp->is : NULL);
    ...
    ffp_dump_p2p_jitter(ffp, mediatype, pts, 150000);
    AVPacket pkt;
    av_init_packet(&pkt);
    if(0 != data_len)
    {
...
int ffp_get_image_jpg(FFPlayer *ffp, char* pdata, int data_len, int64_t* ppts)
{
    if(!ffp || !ffp->is || !ffp->is->pcur_frame)
    {
        av_log(ffp, AV_LOG_ERROR, "ffp:%p, ffp->is:%p\n", ffp, ffp ? ffp->is : NULL);
        return -1;
    }
    int ret = -1;
    SDL_LockMutex(ffp->is->play_mutex);
    AVFrame* pframe = av_frame_clone(ffp->is->pcur_frame);
    if(NULL == pframe)
    {
        lberror("out of memory, clone avframe failed\n");
        return -1;
    }
    ...
    if(AV_PIX_FMT_YUV420P == pframe->format)
    {
        ...
        lbtrace("pframe->format:%d change to AV_PIX_FMT_YUVJ420P\n", pframe->format);
    }
    ret = encoder_frame(AV_CODEC_ID_MJPEG, pframe, pdata, data_len);
    if(ppts)
    {
        *ppts = pframe->pts;
    }
    ...
    return ret;
            ...
            #define AVMUXER_MSG_COMPLETE                2003
            #define AVMUXER_MSG_ERROR                   2004    // arg1 = 0, arg2 = error code
            
            #define FFP_MUXER_MSG                        2000
            #define FFP_MUXER_MSG_START                   2001
            #define FFP_MUXER_MSG_PROGRESS                2002    // arg1 = mediatype, arg2 = pts
            #define FFP_MUXER_MSG_COMPLETE                2003
            #define FFP_MUXER_MSG_ERROR                   2004    // arg1 = 0, arg2 = error code
void ffp_muxer_msg_notify(void* powner, int msgid, int wparam, int lparam)
{
    FFPlayer *ffp = (FFPlayer*)powner;
    if(NULL == ffp)
    {
        lberror("ffp_msg_notify failed, powner is null, msgid:%d, wparam:%d, lparam:%d", msgid, wparam, lparam);
        return ;
    }
    switch(msgid)
    {
        case AVMUXER_MSG_START:
            wparam = FFP_MUXER_MSG_START;
            break;
        case AVMUXER_MSG_PROGRESS:
            wparam = FFP_MUXER_MSG_PROGRESS;
            break;
        case AVMUXER_MSG_COMPLETE:
            wparam = FFP_MUXER_MSG_COMPLETE;
            break;
        case AVMUXER_MSG_ERROR:
            wparam = FFP_MUXER_MSG_ERROR;
            break;
        default:
            wparam = msgid;
            break;
    ...
    ffp_notify_msg3(ffp, FFP_MUXER_MSG, wparam, lparam);
}

void ffp_dump_p2p_jitter(FFPlayer *ffp, int mediatype, int64_t pts, int64_t maxdur)
{
     if(NULL == ffp)
     {
         return ;
    ...
    //sv_trace("mt:%d last pts:%"PRId64", new pts:%"PRId64", duration:%"PRId64"", mediatype, *ppts, pts, pts - *ppts);
    if(*ppts && (pts - *ppts) > maxdur)
    {
        sv_warn("%s pts skip %"PRId64"ms, some frames mybe drop becase network jitter!", AVMEDIA_TYPE_VIDEO == mediatype ? "video":"audio", (pts - *ppts)/1000);
    }
    *ppts = pts;
    return ;
}

void ffp_set_play_audio_callback(FFPlayer *ffp, void* powner, play_audio_callback pcm_cb_func)
{
    if(NULL == ffp)
    {
        sv_error("Invalid parameter, ffp:%p, pcm_cb_func:%p\n", ffp, pcm_cb_func);
        return ;
    }
    SDL_LockMutex(ffp->af_mutex);
    sv_trace("SDL_AoutSetAudioCallback(ffp->aout:%p, pcm_cb_func:%p, powner:%p)\n", ffp->aout, pcm_cb_func, powner);
    SDL_AoutSetAudioCallback(ffp->aout, pcm_cb_func, powner);
    /*ffp->powner = powner;
    ffp->pcm_cb = pcm_cb_func;*/
    SDL_UnlockMutex(ffp->af_mutex);
}

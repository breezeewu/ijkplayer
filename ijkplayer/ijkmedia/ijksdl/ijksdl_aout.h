...
typedef void (*sdl_audio_hook_func)(void* pvoid, char* pdata, int len, int64_t timestamp, int channel, int samplerate, int samformat);
typedef struct SDL_AUDIO_HOOK
{
    sdl_audio_hook_func pcm_hook_func;
    void* powner;
} SDL_Ahook;
...
struct SDL_Aout {
    ...
    void   (*SDL_AoutSetAudioCB)(SDL_Aout *aout, sdl_audio_hook_func* phook_func, void* powner);
    
    int    (*func_get_min_buffer_size)(SDL_Aout *aout);

...
SDL_Android_AudioTrack *SDL_Android_AudioTrack_new_from_spec(JNIEnv *env, SDL_Android_AudioTrack_Spec *spec)
{
    ...
    //__android_log_print(ANDROID_LOG_WARN, "IJKMedia", "atrack->min_buffer_size:%d\n", atrack->min_buffer_size);
    ALOGW("%s: init volume as %f/(%f,%f), atrack->min_buffer_size:%d", __func__, init_volume, atrack->min_volume, atrack->max_volume, atrack->min_buffer_size);
...
int SDL_Android_AudioTrack_write(JNIEnv *env, SDL_Android_AudioTrack *atrack, uint8_t *data, int size_in_byte)
{
    ...
    if(atrack->hook_func)
    {
        atrack->hook_func(atrack->powner, data, (int)size_in_byte, -1, CHANNEL_OUT_MONO == atrack->spec.channel_config ? 1 : 2, atrack->spec.sample_rate_in_hz, ENCODING_PCM_16BIT == atrack->spec.audio_format ? 1 : 8);
    }
...
void SDL_Android_AudioTrack_SetAudioCallback(SDL_Android_AudioTrack *atrack, sdl_audio_hook_func* phook_func, void* powner)
{
	ALOGI("SDL_Android_AudioTrack_SetAudioCallback(atrack:%p, phook_func:%p, powner:%p)\n", atrack, phook_func, powner);
	if(atrack)
	{
		atrack->hook_func = phook_func;
		atrack->powner = powner;
	}
}
...
static void aout_set_audio_callback(SDL_Aout *aout, sdl_audio_hook_func* phook_func, void* powner)
{
	SDL_Aout_Opaque *opaque = aout->opaque;
	ALOGI("aout_set_audio_callback(aout:%p, phook_func:%p, powner:%p), opaque->atrack:%p\n", aout, phook_func, powner, opaque->atrack);
	if(opaque && opaque->atrack)
	{
		ALOGI("SDL_Android_AudioTrack_SetAudioCallback(opaque->atrack:%p, phook_func:%p, powner:%p)\n", opaque->atrack, phook_func, powner);
		SDL_Android_AudioTrack_SetAudioCallback(opaque->atrack, phook_func, powner);
	}
}

static int aout_audio_get_min_buf_size(SDL_Aout *aout)
{
    SDL_Aout_Opaque *opaque = aout->opaque;
	ALOGI("aout_audio_get_min_buf_size(aout:%p), opaque->atrack:%p\n", aout, opaque->atrack);
	if(opaque && opaque->atrack)
	{
		ALOGI("SDL_Android_AudioTrack_get_min_buffer_size(opaque->atrack:%p)\n", opaque->atrack);
		return SDL_Android_AudioTrack_get_min_buffer_size(opaque->atrack);
	}
...
SDL_Aout *SDL_AoutAndroid_CreateForAudioTrack()
{
    ...
    aout->SDL_AoutSetAudioCB = aout_set_audio_callback;
    aout->func_get_min_buffer_size = aout_audio_get_min_buf_size;

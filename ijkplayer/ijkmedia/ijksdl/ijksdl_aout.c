...
double SDL_AoutGetLatencySeconds(SDL_Aout *aout)
{
    if (!aout)
        return 0;
    //ALOGI("SDL_AoutGetLatencySeconds(aout:%p), aout->func_get_latency_seconds:%p, minimal_latency_seconds:%lf\n", aout, aout->func_get_latency_seconds, aout->minimal_latency_seconds);
...
{
    //ALOGI("SDL_AoutSetDefaultLatencySeconds(aout:%p, latency:%lf), aout->func_set_default_latency_seconds:%p\n", aout, latency, aout->func_set_default_latency_seconds);
    if (aout) {
...
void SDL_AoutSetAudioCallback(SDL_Aout *aout, sdl_audio_hook_func* phook_func, void* powner)
{
    if(aout && aout->SDL_AoutSetAudioCB)
    {
        ALOGI("aout->SDL_AoutSetAudioCB(aout:%p, phook_func:%p, powner:%p)\n", aout, phook_func, powner);
        aout->SDL_AoutSetAudioCB(aout, phook_func, powner);
        ALOGI("aout->SDL_AoutSetAudioCB end\n");
    }
}

int SDL_AoutGetMinBufferSize(SDL_Aout *aout)
{
    if(aout && aout->func_get_min_buffer_size)
    {
        int minbufsize = aout->func_get_min_buffer_size(aout);
        ALOGI("minbufsize:%d = aout->func_get_min_buffer_size(aout:%p)\n", aout, minbufsize);

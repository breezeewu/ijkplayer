...
#include "audio_processing/modules/audio_processing/aec/aec_resampler.h"
#include "audio_processing/modules/audio_processing/aec/echo_cancellation_internal.h"
...
void WebRtcAec_enable_delay_correct(void* handle, int enable)
{
  aecpc_t* self = handle;
  if (self == NULL) {
    //self->lastError = AEC_NULL_POINTER_ERROR;
  ...
  WebRtcAec_enable_delay_correction(self->aec, enable);
}
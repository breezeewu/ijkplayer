...
@synthesize currentVideoTimeStamp;
@synthesize playbackMuted;
@synthesize duration;
...
/*#ifdef DEBUG
        ...
        _view = _glView;
        _hudViewController = nil;/*[[IJKSDLHudViewController alloc] init];
...
- (void)prepareToPlay
{
    ...
    //ijkmp_set_data_source(_mediaPlayer, nil);
...
// add by dawson for sunvalley ipc 2.0
- (void)deliverData:(int) mt mediadata:(char*) pdata data_size:(int) size present_time:(long long) pts_in_ms frame_num:(long)frame_num
{
    ijkmp_deliver_packet(_mediaPlayer, mt, pdata, size, pts_in_ms*1000, frame_num);
}

- (void) deliverLoop:(NSString*)purl
{
    NSString *homePath = NSHomeDirectory();
    //NSLog(@"home根目录:%@", homePath);
    //2.获取Documents路径
    ...
    while(precord_demux)
    {
        int mt = -1;
        long pts = 0;
        int size = read_frame(precord_demux, &mt, pbuf, buf_len, &pts, &frame_num);
        if(size > 0)
        {
            [self deliverData:mt mediadata:pbuf data_size:size present_time:pts frame_num:frame_num];
...
- (int) getImageJpg:(char*)pdata dataLen:(int)datalen
{
    int data_len = ijkmp_get_image_jpg(_mediaPlayer, pdata, datalen, NULL);
    
    return data_len;
}
- (int) saveImageJpg:(NSString*)imgurl
{
    return ijkmp_save_image_jpg(_mediaPlayer, [imgurl UTF8String]);
}

- (void)startRecord:(NSString*)sinkUrl tmpUrl:(NSString*)tmpurl
{
    ijkmp_start_record(_mediaPlayer, [sinkUrl UTF8String], [tmpurl UTF8String]);
}

- (void)stopRecord:(BOOL)bcancel
{
    ijkmp_stop_record(_mediaPlayer, bcancel);
}

- (void)openRecord:(NSString*)purl
{
    precord = record_open([purl UTF8String]);
}
- (int)readFrame:(char*)pdata frameSize:(int)size mediaType:(int*)mt presentTime:(long long*)ppts_in_ms frame_num:(long)frame_num
{
    return read_frame(precord, mt, pdata, &size, ppts_in_ms, frame_num);
}

- (void)closeRecord
{
    record_close(precord);
}
/*int            ijkmp_start_echo_cancel(IjkMediaPlayer *mp, int channel, int samplerate, int sampleformat, int nbsamples);
...
int            ijkmp_echo_cancel(IjkMediaPlayer *mp, char* pdata, char* pout, int len, int64_t timestamp);

int            ijkmp_stop_echo_cancel(IjkMediaPlayer *mp);*/
- (void)startEchoCancel:(int)channel samplerate:(int)samplerate sampleformat:(int)format nbsamples:(int)nbsamples delay:(int)usdelay
{
    ijkmp_start_echo_cancel(_mediaPlayer, channel, samplerate, format, nbsamples, usdelay);
}
- (int)echoCancelDeliverData:(char*)pdata outdata:(char*)pout data_len:(int)len
{
    return ijkmp_echo_cancel(_mediaPlayer, pdata, pout, len);
}
- (void)stopEchoCancel
{
    ijkmp_stop_echo_cancel(_mediaPlayer);
}
// add end
...
- (void)stop
{
    ...
    [self stopHudTimer];
    [self performSelectorInBackground:@selector(stop_intel) withObject:nil];
...
+ (void)setLog:(NSString*) path loglevel:(int)logLevel outputflag:(int)outputflag
{
    //ijkmp_global_set_log_level(logLevel);
    ijkmp_global_init_log([path UTF8String], logLevel, outputflag);
}
...
- (long)currentVideoTimeStamp
{
    ...
    return ijkmp_get_current_video_timestamp(_mediaPlayer);
}

- (void)setPlaybackMuted:(BOOL)bplaybackMuted
{
    if (!_mediaPlayer)
        return;

    ijkmp_set_playback_mute(_mediaPlayer, (int)bplaybackMuted);
}
...
- (BOOL)playbackMuted
{
    ...
    return (BOOL)ijkmp_get_playback_mute(_mediaPlayer);
}
...
- (BOOL)isRecording
{
    ...
    return ijkmp_is_recording(_mediaPlayer) == true ? TRUE : FALSE;
}
...
- (void)postEvent: (IJKFFMoviePlayerMessage *)msg
{
    ...
    switch (avmsg->what) {
        ...
        case FFP_MUXER_MSG:
        {
            switch(avmsg->arg1)
            {
                case FFP_MUXER_MSG_START:
                    [[NSNotificationCenter defaultCenter]
                     postNotificationName:IJKMediaMuxerStartNotification
                     object:self];
                    break;
                case FFP_MUXER_MSG_PROGRESS:
                    [[NSNotificationCenter defaultCenter]
                     postNotificationName:IJKMediaMuxerProgressNotification
                     object:self
                     userInfo:@{IJKMediaMuxerProgressNotificationPTS: @(avmsg->arg2)}];
                    break;
                case FFP_MUXER_MSG_COMPLETE:
                    [[NSNotificationCenter defaultCenter]
                     postNotificationName:IJKMediaMuxerCompleteNotification
                     object:self
                     userInfo:@{IJKMediaMuxerCompleteNotificationDuration: @(avmsg->arg2)}];
                    break;
                case FFP_MUXER_MSG_ERROR:
                    [[NSNotificationCenter defaultCenter]
                     postNotificationName:IJKMediaMuxerErrorNotification
                     object:self
                     userInfo:@{IJKMediaMuxerErrorNotificationReason: @(avmsg->arg2)}];
                    break;
                case FFP_MUXER_MSG_INTERRUPT:
                    [[NSNotificationCenter defaultCenter]

...
static void IJKSDLAudioQueueOuptutCallback(void * inUserData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer) {
    @autoreleasepool {
        ...
        /*if(aqController->pcm_cb)
        {
            //(void* pvoid, char* pdata, int len, int64_t timestamp, int channel, int samplerate, int samformat);
            aqController->pcm_cb(aqController->powner, -1, _spec->channels, _spec->freq, _spec->format);
        }*/
        //[aqController audioHook:inBuffer];
        AudioQueueEnqueueBuffer(inAQ, inBuffer, 0, NULL);
    }
}

- (void)setAudioCallback:(sdl_audio_hook_func)hook_func owner:(void*)powner
{
    self->pcm_cb = hook_func;
    self->powner = powner;
}


/*- (void)audioHook:(AudioQueueBufferRef)pabuff
{
    if(pcm_cb)
    {
        pcm_cb(powner, pabuff->mAudioData, pabuff->mAudioDataByteSize, -1, _spec.channels, _spec.freq, 1);
    }
}*/
@end

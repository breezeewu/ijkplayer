...
#define lazy_extra_function_support
#ifdef lazy_extra_function_support
//add by zwu
static int has_media_stream(AVFormatContext *ic, int mt)
{
    for(int i = 0; i < ic->nb_streams; i++)
    {
        if(mt == ic->streams[i]->codecpar->codec_type && has_codec_parameters(ic->streams[i], NULL))
        {
            return 1;
        }
    }
...
static int is_continue_probe_stream(AVFormatContext *ic, int64_t probesize, int64_t readsize)
{
    if((has_media_stream(ic, AVMEDIA_TYPE_VIDEO) && has_media_stream(ic, AVMEDIA_TYPE_AUDIO)) || readsize >= probesize)
    {
        av_log(ic, AV_LOG_DEBUG, "is_continue_probe_stream has_video_audio && readsize >= probesize return 0\n");
        return 0;
    }
    av_log(ic, AV_LOG_DEBUG, "is_continue_probe_stream return 1\n");
    return 1;
}
//add end
#endif
...
#ifdef lazy_extra_function_support
        if(is_continue_probe_stream(ic, probesize, read_size) == 0)//change by zwu
#else

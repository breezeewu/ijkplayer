...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    public static final int media_codec_id_h264 = 28;
    public static final int media_codec_id_h265 = 174;
    public static final int media_codec_id_aac = 86018;
    ...
    private static final int MEDIA_PLAYBACK_STATE = 700;
    ...
    public void setDataSource(String path)
        ...
        /*if(path == null)
        {
            mDataSource = "play_raw_data";
            _setDataSource(mDataSource, null, null);
            return ;
        }*/
        //DebugLog.d("ijkplayer", path);
        mDataSource = path;
    ...
    private static class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
            case MEDIA_PREPARED:
                System.out.print("MEDIA_PREPARED *******");
            ...
            case MEDIA_INFO:
                switch (msg.arg1) {
                    ...
                    /*case MEDIA_INFO_PLAYBACK_STATE_CHANGED:
                        DebugLog.i(TAG, "Info: MEDIA_INFO_PLAYBACK_STATE_CHANGED state:"+msg.arg2);
                        break;*/
                }
            ...
            case MEDIA_SET_VIDEO_SAR:
                ...
                break;
            // add by dawson
            case MEDIA_PLAYBACK_STATE:// 700
                player.mPlayBackState = msg.arg1;
                switch(msg.arg1)
                {
                    case 0:
                        System.out.print("IJKPlayer state idle\n");
                        break;
                    case 1:
                        System.out.print("IJKPlayer state initialize\n");
                        break;
                    case 3:
                        System.out.print("IJKPlayer state preparing\n");
                        break;
                    case 4:
                        System.out.print("IJKPlayer state prepared\n");
                        break;
                    case 5:
                        System.out.print("IJKPlayer state started\n");
                        break;
                    case 6:
                        System.out.print("IJKPlayer state paused\n");
                        break;
                    case 7:
                        System.out.print("IJKPlayer state completed\n");
                        break;
                    case 8:
                        System.out.print("IJKPlayer state stoped\n");
                        break;
                    case 9:
                        System.out.print("IJKPlayer state error\n");
                        break;
                    default:
                        System.out.print("IJKPlayer state unknown\n");
                        break;
                }
                break;
            case MEDIA_MUXER_MSG:
                switch(msg.arg1)
                {
                    ...
                    case IMediaPlayer.MEDIA_MUXERT_MSG_PROGRESS:
                        ...
                        System.out.print(msg.arg2);
                        break;
                    case IMediaPlayer.MEDIA_MUXERT_MSG_COMPLETE:
                        System.out.print("MEDIA_MUXERT_MSG_COMPLETE, duration:");
                        System.out.print(msg.arg2);
                        break;
                    case IMediaPlayer.MEDIA_MUXERT_MSG_ERROR:
                        ...
                        System.out.print(msg.arg2);
                        break;
                    case IMediaPlayer.MEDIA_MUXER_MSG_INTERRUPT:
                        System.out.print("MEDIA_MUXER_MSG_INTERRUPT:");
                        System.out.print(msg.arg2);
                        break;
                    default:
    ...
    public static native void native_setLogPath(String path, int level, int outputflag);

    public native void _setPlaybackMute(int muted);

    public native int _getPlaybackMute();

    public native static String nativeGetVersion();
    // int mt:media type, video:0, audio 1
    // int codec_id: video codec:h.264:media_codec_id_h264/28, h.265:media_codec_id_h265/174, audio codec:aac:media_codec_id_aac:86018
    // byte[] encData, media codec data
    // long pts media timestamp
    //public native void nativeDeliverPacket(int mt, byte[] encData, long pts);
    public native void nativeDeliverPacket(int codec_id, byte[] encData, long pts, long frame_num);
    //public static native void nativeDeliverPacket(int mt, long pts);
    public native int nativeSaveCurrentImage(String imgPath);

    public native void nativeStartRecord(String recordPath, String tmpPath);

    public native void nativeStopRecord(int bcancel);

    public native long nativeGetCurrentVideoTimeStamp();
    ...
    //public native byte[] nativeEchoCancelDeliverDataEx(byte[] micdata, byte[] sddata);
    public native void nativeStopEchoCancel();

    public static native long nativeEncoderOpen(int mt, int codecId, int channel, int samplerate, int format, int bitrate);
    public static native byte[] nativeEncoderDeliverData(long encId, byte[] rawData, long pts);
    public static native void nativeEncoderClose(long encId);
    public static native byte[] nativeConvertKeyframeToJPG(int codec_id, byte[] h264data);
    //public static native int nativeConvertKeyframeToJPGFile(byte[] h264data, String jpgpath);
    //public static native int nativeConvertH264ToJPGFile(byte[] h264data, String jpgpath);
    public static native int nativeConvertKeyframeToJPGFile(int codec_id, byte[] h264data, String jpgpath);

    /*public void setPlaybackMute(int muted)
    {
        _setPlaybackMute(muted);
    }

    public int getPlaybackMute()
    {
        return _getPlaybackMute();
    }

    public void DeliverPacket(int codec_id, byte[] encData, long pts)
    {
        nativeDeliverPacket(codec_id, encData, pts);
    }

    public int GetCurrentImage(String imgPath)
    {
        return nativeSaveCurrentImage(imgPath);
    }

    public void StartRecord(String recordPath, String tmpPath)
    {
        nativeStartRecord(recordPath, tmpPath);
    }

    public void StopRecord(int bcancel)
    {
        nativeStopRecord(bcancel);
    }*/

    @Override
    public int StartEchoCancel(int channel, int samplerate, int sampleformat, int nbsamples, int usdelay)
    {
        return nativeStartEchoCancel(channel, samplerate, sampleformat, nbsamples, usdelay);
    }

    @Override
    public byte[] EchoCancelDeliverData(byte[] pcmdata)
    {
        //return pcmdata;
        return nativeEchoCancelDeliverData(pcmdata);
    }

    @Override
    public long GetVideoTimeStamp()
    {
        return nativeGetCurrentVideoTimeStamp();
    }

    @Override
    public void StopEchoCancel()
    {
        nativeStopEchoCancel();
    }
    ...
    public void DeliverPacket(int mt, byte[] encData, long pts, long framenum)
    {
        //System.out.print("mMediaPlayer.DeliverPacket("+mt+"encData"+pts+")");
        //Log.d(TAG, "mMediaPlayer.DeliverPacket("+mt+"encData"+pts+")");
        nativeDeliverPacket(mt, encData, pts, framenum);
    }
    ...
    public long EncoderOpen(int mt, int codecId, int channel, int samplerate, int format, int bitrate)
    {
        return nativeEncoderOpen(mt, codecId, channel, samplerate, format, bitrate);
    }

    public byte[] EncoderDeliverData(long encId, byte[] rawData, long pts)
    {
        return nativeEncoderDeliverData(encId, rawData, pts);
    }

    public void EncoderClose(long encId)
    {
        nativeEncoderClose(encId);
    }
